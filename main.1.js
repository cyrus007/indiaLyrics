...
GEETURL = "http://lyricsindia.net/songs/search?txt=#{title}&format=xml";
...
INDIAERR = "There is some problem with site http://indialyrics.net/";
...
Amarok.debug( response );
            if(capture = /sorry,/i.exec(response)) {
                Amarok.Lyrics.showLyricsError( INDIAERR );
                return;
            }
            var doc = new QDomDocument();
            doc.setContent( response );

            var lyrics = doc.elementsByTagName( "unidata" ).item( 0 ).toElement().text();
            var title = doc.elementsByTagName( "unititle" ).item( 0 ).toElement().text();
            var node = doc.elementsByTagName( "movie" ).item( 0 );
            var album = node.firstChildElement( "name" ).text();
            var year  = node.firstChildElement( "year" ).text();
            var nodelist = doc.elementsByTagName( "singer" );
            var singer = "";
            for( var i = 0; i < nodelist.length(); i++ ) {
                singer = singer + ( singer ? ', ': '' ) + nodelist.item( i ).firstChildElement( "name" ).text();
            }
            var nodelist = doc.elementsByTagName( "composer" );
            var composer = "";
            for( i = 0; i < nodelist.length(); i++ ) {
                composer = composer + ( composer ? ', ': '' ) + nodelist.item( i ).firstChildElement( "name" ).text();
            }
            var nodelist = doc.elementsByTagName( "lyricist" );
            var lyricist = "";
            for( i = 0; i < nodelist.length(); i++ ) {
                lyricist = lyricist + ( lyricist ? ', ' : '' ) + nodelist.item( i ).firstChildElement( "name" ).text();
            }
            ...
            NEWHTML = NEWHTML.replace( "{links}", "&copy; <a href='http://www.lyricsindia.net/'>LyricsIndia</a>" );
...
{
    ITRANSURL = "http://lyricsindia.net/songs/show/";
    try
    {
        if( response.length == 0 )
            Amarok.Lyrics.showLyricsError( "Unable to contact server - no website returned" ); // TODO: this should be i18n able
        else
        {
            ...
            song_no = title = suggestion = "";
            var nodelist = doc.elementsByTagName( "song" );
            var maxno = nodelist.length();
            if( maxno < 1 ) {
                Amarok.Lyrics.showLyricsNotFound( "No lyrics were found for this title." );
                return;
            }
            else if( maxno == 1 ) {
                song_no = nodelist.at( 0 ).toElement().text();
                var url = QUrl.fromEncoded( new QByteArray( ITRANSURL + song_no + ".xml" ), 1);
                Amarok.debug( "URL: " + url.toString() );
                new Downloader( url, new Function("response", "onIndoLyricsReceived(response, -1)") );
            }
            else { // handle multiple records here
                for( var i = 0; i < maxno; i++ ) {
                    song_no = nodelist.item( i ).firstChildElement( "id" ).text();
                    title = nodelist.item( i ).firstChildElement( "unititle" ).text();
                    suggestion = suggestion + "<suggestion title=\"" + title + "\" url=\"" + ITRANSURL + song_no + ".xml\" />\n";
                }
...
Amarok.alert( "title: " + title );
                var newurl = GEETURL.replace( "#{title}", encodeURIComponent(title) );
                //newurl = "http://localhost/itrans/929.xml";
